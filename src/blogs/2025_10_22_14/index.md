...
* Workspace:工作区，就是你平时存放项目代码的地方(和.git并行的文件)
* Index/Stage:暂存区，用于临时存放你的改动，事实上它只是一个文件，保存即将提交到文件列表信息(会在.git中生成一个文件index，存放暂存文件的信息)
* Repository:仓库区（或本地仓库），就是安全存放数据的地方，这里面有你提交到所有版本的数据。其中HEAD指向最新放入仓库的版本(可以说就是.git文件，里面的objects保存着每次提交的信息，通过checkout可以回滚到之前的版本)
* Remote:远程仓库，托管代码的服务器，可以简单地认为是你项目组中的一台电脑用于远程数据交换(比如说对于本地来说，码云、github等都是远程服务器，在github上创建仓库，相当于将仓库托管在外部服务器)
...
git add 文件名.后缀 文件名.后缀
git add . #提交全部文件
git add --all #提交全部文件
git commit -m "第一次提交"
git status #查看用于查看当前工作区、暂存区与本地版本库的状态差异
git log #每次提交的版本信息
git log --pretty=oneline #信息简约到一行
git pull #拉取远程仓库
git push #推送远程仓库
...
![image-20251023154227922](./12.png)

最后一行展示的是add后index.md中的内容

在.git/objects/e6中也会多一个文件

![image-20251023160555189](./14.png)

```
git commit -m "第一次提交"
```

![image-20251023160453539](./13.png)

![image-20251023160611821](./15.png)

![image-20251023160807258](./16.png)

当前所在分支的指针（如 `main` 或 `master`）会被更新，指向刚创建的新提交对象,存放在.git/pbjects中。

- 分支指针文件位于 `.git/refs/heads/` 目录下，例如 `.git/refs/heads/main`，文件内容会从之前的旧提交哈希值，替换为本次新提交的哈希值。

![image-20251023162619810](./17.png)

```
git checkout 哈希码 #回滚到哪一次提交
```

每次提交的版本都保存在.git/objects中

![image-20251023164242199](./18.png)

![image-20251023175008172](./19.png)

如果同一个文件再合并分支时都修改了则会引起冲突：解决办法是修改冲突文件后再提交

==master主分支应该非常稳定，用来发布新版本，一般情况下不予许在上面工作，工作情况下在新建的dev分支上工作，工作完后，比如要发布，或者说dev分支代码稳定后可以合并到主分支master上来==
...
# 若未关联，执行关联（远程仓库地址需替换为你的仓库地址）
git remote add origin https://gitee.com/你的用户名/你的仓库名.git
# 或 GitHub 地址：https://github.com/你的用户名/你的仓库名.git
```

```
# 首次推送当前分支到远程（-u 表示关联分支，后续可简化命令）
...
![image-20251023175814729](./20.png)
